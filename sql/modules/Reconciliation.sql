...
CREATE OR REPLACE VIEW recon_payee AS
     SELECT n.name AS payee, rr.id, rr.report_id, rr.scn, rr.their_balance,
            ...
            rr.trans_type, rr.post_date, rr.ledger_id, ac.voucher_id, rr.overlook, rr.cleared
       FROM cr_report_line rr
       LEFT JOIN acc_trans ac ON rr.ledger_id = ac.entry_id
       LEFT JOIN gl ON ac.trans_id = gl.id
       LEFT JOIN (
            SELECT aa.id, e.name FROM (
                    SELECT id, entity_credit_account AS eca FROM ap
              UNION SELECT id, entity_credit_account AS eca FROM ar
            ) aa
            JOIN entity_credit_account eca ON aa.eca = eca.id
            JOIN entity e ON eca.entity_id = e.id
        UNION
            SELECT gl.id, gl.description FROM gl
