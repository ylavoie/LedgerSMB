...
sub is_hidden {
    ...
    return $self->_logged(
        sub {
            return $self->driver->is_hidden($element->_id);
        },
        'is_hidden', 'query is_hidden');
}

=item is_enabled($element)

Returns a boolean value indicating if the element identified by
C<$element> is enabled on the page, i.e. that it can be used
for interaction.
...
sub is_enabled {
    ...
    return $self->_logged(
        sub {
            return $self->driver->is_enabled($element->_id);
        },
